function showTrending() {
   function createTrendingEl() {
      let trendingEl = document.createElement('div');
      trendingEl.className = 'trending';
      ...
      document.querySelector('.main').append(trendingEl);

      getDataForTrending();
   }

   async function getDataForTrending() {
      try {
   ...
   function renderTrending(films) {
      ...
      films.forEach((film) => {
         if (film.isTrending) {
            let filmsCard = `
               <div class="swiper-slide">
                  <div class="card" data-id=${film.id}>
                     <div class="card__bookmark${(dataSeries && dataSeries.includes(film.id)) || (dataMovies && dataMovies.includes(film.id)) ? ' bookmarked' : ''}">
                     ...
                     <div class="card__img">
                        <picture>
                           <source
                              ...
                              srcset="${film.thumbnail.trending.large}">
                           <source
                              media="(max-width: 767px)"
                              srcset='${film.thumbnail.trending.small}'>
                           <img src="${film.thumbnail.trending.small}" alt="Film">
                        </picture>
                     </div>
            ...
            `;
            document.querySelector('.swiper-wrapper').insertAdjacentHTML('beforeend', filmsCard);
         }
      });
   }
